...
import io.sharedstreets.tools.builder.osm.tools.GeometryTools;
...
public class BaseSegment extends SpatialEntity {
    ...
    public static boolean canMerge(BaseSegment baseSegment1, BaseSegment baseSegment2) {
        ...
        // if oneway or roundabout (implicitly oneway or...ack!) both sections need to
        ...
        if ((baseSegment1.oneWay && baseSegment2.oneWay) || (baseSegment1.roundabout && baseSegment2.roundabout)) {
            if (!baseSegment2.getFirstNode().equals(baseSegment1.getLastNode())
                    && !baseSegment1.getFirstNode().equals(baseSegment2.getLastNode()))
                return false;
        }
        ...
        if (baseSegment1.waySections.length == baseSegment2.waySections.length) {
            ...
            for (int i = 0; i < baseSegment1.waySections.length; i++) {
                if (!baseSegment1.waySections[i].wayId.equals(baseSegment2.waySections[i].wayId)
                        && !baseSegment1.waySections[i].nodes[0].nodeId
                                .equals(baseSegment2.waySections[i].nodes[0].nodeId)
                        && !baseSegment1.waySections[i].nodes[baseSegment1.waySections[i].nodes.length - 1].nodeId
                                .equals(baseSegment2.waySections[i].nodes[baseSegment2.waySections[i].nodes.length
                                        - 1].nodeId)) {
                    duplicate = false;
                }
            }
        ...
        // Check line length attempt
        Geometry g1 = GeometryTools.constructGeometryTool(baseSegment1.waySections);
        Geometry g2 = GeometryTools.constructGeometryTool(baseSegment2.waySections);
        double g1Length = g1.calculateLength2D();
        double g2Length = g2.calculateLength2D();

        System.out.println("g1Length" + g1Length);
        System.out.println("g2Length" + g2Length);
    ...
    public Way.ROAD_CLASS getRoadClass() {
        ...
        for (WaySection waySection : this.waySections) {
            ...
            // per
            // https://github.com/sharedstreets/sharedstreets-ref-system/issues/20#issuecomment-378079937
            // [excluded road]
            // |
